...
# Change FROM_DATE to go back further (format: YYYY-MM-DD).
# The Guardian API supports any date range.
# LLM web search runs one query per player per month, so a longer range means more months
# and more API calls — expect roughly 6 players × N months requests.
