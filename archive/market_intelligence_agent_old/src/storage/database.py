...
class EventDatabase:
    ...
    # ========================================================================
    # CONTENT FETCH OPERATIONS (for Content Harvester V2)
    # ========================================================================
    ...
    def get_latest_content_hash(self, url: str) -> Optional[str]:
        ...
        Used by Content Harvester V2 for change detection.

        Args:
            url: URL to check

        Returns:
            Content hash string or None if never fetched
        """
        with self._get_connection() as conn:
            cursor = conn.cursor()
    ...
    def store_content_fetch(
        ...
        """
        Store a content fetch record.

        Used by Content Harvester V2 for change detection and history tracking.

        Args:
            url: URL that was fetched
            content_hash: SHA256 hash of content
            fetched_at: Timestamp of fetch
            is_noise: Whether content was classified as noise
            provider: Provider name
            raw_content: Optional raw content text
            content_category: Optional category (article, changelog, etc.)
            noise_confidence: Optional confidence score (0-1)
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute("""
                INSERT OR IGNORE INTO sources (url, provider, first_discovered)
                VALUES (?, ?, ?)
            """, (url, provider, datetime.now().isoformat()))

            # Store fetch record
            cursor.execute("""
                INSERT INTO content_fetches (
                    url, fetched_at, content_hash, raw_content,
                    fetch_successful, content_type, metadata_json
                )
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (
                url,
                ...
                content_hash,
                raw_content,
                not is_noise,  # fetch_successful = True if not noise
                content_category,
